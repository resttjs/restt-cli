...
const startService = (worker) => {
  ...
  if (configuration.workerskv && configuration.workerskv.namespaces) {
    ...
    for (const namespace of configuration.workerskv.namespaces) {
      ...
      if (['console', 'cache', 'process', 'configuration'].indexOf(namespace.toLowerCase()) > -1) {

        // Throw an error as the namespace is one of the restricted keys
        throw(`${failed}\n\nWorkersKV: Namespace cannot be called '${namespace}'`);
      }

      // Check if the namespace contains anything other than letters
      if (namespace.match(/[^\w]/ig)) throw(`${failed}\n\nWorkersKV: Namespace must only contain letters`);

      // If the namespace is in lowercase
      if (namespace != namespace.toUpperCase()) log({
        description: `WorkersKV: Namespace '${namespace}' should be uppercase - automatically casted to '${namespace.toUpperCase()}'`,
        type: 'warning',
        label: false,
        clear: true
      });
      ...
      // Bind the namespace
      options.bindings[namespace.toUpperCase()] = new KeyValueStore();
    }
  }
  ...
  portcheck(port, () => {
    ...
    // Catch any errors from portcheck
    tryCatch(() => { 

      // Throw the error message as we cannot start the service
      throw(`${failed}\n\n${String(error)}`); 
    });
  });
