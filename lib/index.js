...
module.exports = ({ action, script }) => {
  ...
  if (action == 'deploy') {
    ...
    webpack.production(sourceScript, () => {
      ...
      cloudflare.upload(outputScript, (success) => {

        // Check if we have any cloudflare workerskv namespaces
        if (success && (configuration.workerskv && configuration.workerskv.namespaces && configuration.workerskv.namespaces.length > 0)) {

          // Attempt to create the namespaces
          cloudflare.createNamespaces(configuration.workerskv.namespaces);
        }
